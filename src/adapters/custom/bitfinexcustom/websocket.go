...
func NewWebSocketService(c *bitfinex.Client) *WebSocketService {
...
func (w *WebSocketService) ConnectPrivate(ch chan TermData) {
	...
	connectMsg, _ := json.Marshal(&privateConnect{
		...
		AuthSig:     signPayload(payload, w.client),
		AuthPayload: payload,
	})
...
func signPayload(payload string, c *bitfinex.Client) string {
